...
К сожалению, при помощи `setkeycodes(1)` не удастся поменять
привязанный к клавише скан-код. Другое дело, что в `input.h` заняты
коды, которые с сабжевой клавиатурой не используются. Можно поменять
встроенные в ядро привязки скан-кодов к кодам клавиш на другие,
поменьше 255 (но так, чтобы не было коллизий с другими кнопками на
клавиатуре).
...
Об этом — [в следующей записи][].
  ...
  [в следующей записи]: http://dzhus.org/posts/2008-04-15-using-extra-keys.html
